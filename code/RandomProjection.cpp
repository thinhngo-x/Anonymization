...
Eigen::MatrixXd RandomProjection::RandomGaussianMatrix(int d, int projection_dim) {
    ...
    std::normal_distribution<double> distribution(0,1.0/std::sqrt(projection_dim));
    ...
    for(int i = 0; i < d; i++)
    {
        for(int j = 0; j < projection_dim; j++)
        {
            ...
            // std::cout<<projection_matrix(i,j)<<std::endl;
        }
    }
...
RandomProjection::RandomProjection(int original_dimension, int col_class, int projection_dim, std::string type_sample) {
    ...
    else m_projection = RandomRademacherMatrix(original_dimension, projection_dim).cast<double>();
}
...
Dataset RandomProjection::Project(Dataset *dataset) {
    ...
    // std::cout<<"foo: "<<projected_data(0,8)<<std::endl;
	...
	_dataset.resize(projected_data.rows(), std::vector<double>(projected_data.cols()+1, 0));
    // Copy each element
	for(size_t i = 0; i < _dataset.size(); i++)
		for(size_t j = 0; j < _dataset.front().size(); j++)
        {
			if (j==0)
                _dataset[i][j] = data(i,j);
            else
